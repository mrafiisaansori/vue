      ...
      <table class="table table-hover" v-if="cart.length">
        ...
        <tbody>
          <tr v-for="(item, index) in cart" :key="item.product.id">
            ...
            <td scope="text-center">{{ item.qty }}</td>
            <td scope="text-right">{{ Number(item.product.price) }}</td>
            <td scope="text-right">{{ Number(item.product.price*item.qty) }}</td>
          </tr>
        </tbody>
      </table>
      <router-link class="btn btn-small btn-outline-info text-dark" to="/">Back to Shop</router-link>
    </div>
</template>
